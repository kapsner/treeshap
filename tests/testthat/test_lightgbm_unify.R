...
if (
  ...
  data <- as.matrix(na.omit(data.table::as.data.table(cbind(
    data_fifa,
    fifa20$target
  ))))
  ...
  test_that('basic columns after lightgbm.unify are correct', {
    ...
    to_test_features <- lgbmtree[
      order(lgbmtree$split_index),
      .(split_feature, split_index, threshold, leaf_count, internal_count),
      tree_index
    ]
    expect_equal(
      to_test_features[!is.na(to_test_features$split_index), ][['split_index']],
      unified_model[!is.na(unified_model$Feature), ][['Node']]
    )
    expect_equal(
      to_test_features[['split_feature']],
      unified_model[['Feature']]
    )
    expect_equal(to_test_features[['threshold']], unified_model[['Split']])
    expect_equal(
      to_test_features[!is.na(internal_count), ][['internal_count']],
      unified_model[!is.na(unified_model$Feature), ][['Cover']]
    )
  })
  ...
  test_that('connections between nodes and leaves after lightgbm.unify are correct', {
    ...
    expect_equal(
      test_object[test_object[!is.na(test_object$Yes)][['Yes']]][['Cover']] +
        test_object[test_object[!is.na(test_object$No)][['No']]][['Cover']],
      test_object[!is.na(Feature)][['Cover']]
    )
    #check if default_left information is correctly used
    df_default_left <- lgbmtree[
      default_left == "TRUE",
      c('tree_index', 'split_index')
    ]
    test_object_actual_default_left <- test_object[
      Yes == Missing,
      c('Tree', 'Node')
    ]
    ...
    expect_equal(
      test_object_actual_default_left[order(tree_index, split_index)],
      df_default_left[order(tree_index, split_index)]
    )
    #and default_left = FALSE analogically:
    df_default_right <- lgbmtree[
      default_left != 'TRUE',
      c('tree_index', 'split_index')
    ]
    test_object_actual_default_right <- test_object[
      No == Missing,
      c('Tree', 'Node')
    ]
    ...
    expect_equal(
      test_object_actual_default_right[order(tree_index, split_index)],
      df_default_right[order(tree_index, split_index)]
    )
  ...
  prepare_test_preds <- function(unify_out) {
    ...
    test_obs <- lapply(table(test_tree$Tree), function(y) {
      sample(c(-1, 0, 1), y, replace = T)
    })
    test_tree <- split(test_tree, test_tree$Tree)
    determine_val <- function(obs, tree) {
      root_id <- tree[['node_row_id']][1]
      tree[, c('Yes', 'No', 'Missing')] <- tree[, c('Yes', 'No', 'Missing')] -
        root_id +
        1
      i <- 1
      indx <- 1
      while (!is.na(tree$Feature[indx])) {
        indx <- ifelse(
          obs[i] == 0,
          tree$Missing[indx],
          ifelse(obs[i] < 0, tree$Yes[indx], tree$No[indx])
        )
    ...
    determine_val <- function(obs, tree) {
      ...
      while (!is.na(tree$split_feature[indx])) {
        ...
        if ((obs[i] < 0) | (tree$default_left[indx] == 'TRUE' & obs[i] == 0)) {
          ...
          (obs[i] > 0) | (tree$default_left[indx] == 'FALSE' & obs[i] == 0)
        ) {
  ...
  test_that("LightGBM: mean prediction calculated using predict == using covers", {
    ...
    intercept_covers <- sum(leaves$Prediction * leaves$Cover) /
      sum(leaves$Cover) *
      ntrees
